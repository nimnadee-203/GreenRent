...
async function runImprovedRouteTests() {
    try {
        console.log('--- Final Route-Level Integration Test Started ---');
        ...
        // Generate a unique date based on current time to avoid conflicts
        const uniqueSuffix = Date.now().toString().slice(-4);
        const checkIn = `2028-01-01`;
        const checkOut = `2028-01-05`;
        ...
        const cancelRes = await axios.put(`${BASE_URL}/api/bookings/${bookingId}/cancel`, { cancellationReason: 'Test Success' }, { headers: headers(userToken) });
        ...
        console.log('\n✅ 100% ENDPOINTS VERIFIED SUCCESSFULLY');
